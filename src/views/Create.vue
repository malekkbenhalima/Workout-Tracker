<template>
  <div class="max-w-screen-md mx-auto px-4 py-10">
    ...
    <!-- Create -->
    <div class="p-8 flex items-start bg-light-grey rounded shadow-lg">
      <!-- Form -->
        ...
        <div v-if="workoutType === 'strength'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises"
            ...
            <div class="flex flex-col md:w-1/3">
              ...
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="exercise-name"
                     v-model="item.exercise">
            </div>

            <div class="flex flex-col md:flex-1">
              <label for="sets" class="mb-1 text-sm text-at-light-green">Sets</label>
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="sets"
                     v-model="item.sets">
            </div>

            <div class="flex flex-col md:flex-1">
              <label for="reps" class="mb-1 text-sm text-at-light-green">Reps</label>
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="reps"
                     v-model="item.reps">
            </div>

            <div class="flex flex-col md:flex-1">
              <label for="weight" class="mb-1 text-sm text-at-light-green">Weight (KG)</label>
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="weight"
                     v-model="item.weight">
            </div>
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises"
            ...
            <div class="flex flex-col md:flex-1">
              ...
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="distance"
                     v-model="item.sets">
            </div>

            <div class="flex flex-col md:flex-1">
              <label for="duration" class="mb-1 text-sm text-at-light-green">Duration</label>
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="duration"
                     v-model="item.duration">
            </div>

            <div class="flex flex-col md:flex-1">
              <label for="pace" class="mb-1 text-sm text-at-light-green">Pace</label>
              <input required type="text" class="p-2 w-full text-gray-500 focus:outline-none" id="pace"
                     v-model="item.pace">
            </div>
